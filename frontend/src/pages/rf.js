...
import { pred_rf } from '../services/Apis';
...
const RF = () => {
  ...
  const makePrediction = async (input) => {
    try {
      ...
      const responseAllFeatures = await pred_rf({gender, haemoglobin,mch, mchc, mcv,features: 'all',});
  ...
  const makeoptPrediction = async (input) => {
    try {
      ...
      const responseOptimized = await pred_rf({gender,haemoglobin,mcv,features:'optimized'});
    ...
    <section>
      <div className="form_data">
        <div className="form_heading">
          ...
          <p>Ensemble learning method with 100% accuracy using multiple decision trees</p>
        </div>
