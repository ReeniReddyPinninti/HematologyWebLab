...
import { pred_lr } from '../services/Apis';
...
const LR = () => {
  ...
  const makePrediction = async (input) => {
    try {
      ...
      const responseAllFeatures = await pred_lr({gender, haemoglobin,mch, mchc, mcv,features: 'all',});
  ...
  const makeoptPrediction = async (input) => {
    try {
      ...
      const responseOptimized = await pred_lr({gender,haemoglobin,mcv,features:'optimized'});
    ...
    <section>
      <div className="form_data">
        <div className="form_heading">
          ...
          <p>Statistical learning model with 93.5% accuracy for numerical prediction analysis</p>
        </div>
