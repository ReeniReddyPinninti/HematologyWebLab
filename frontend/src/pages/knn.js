...
const KNN = () => {
  ...
  const makePrediction = async (input) => {
    try {
      ...
      const responseAllFeatures = await pred_knn({gender, haemoglobin,mch, mchc, mcv,features: 'all',});
  ...
  const makeoptPrediction = async (input) => {
    try {
      ...
      const responseOptimized = await pred_knn({gender,haemoglobin,mcv,features:'optimized'});
    ...
    <section>
      <div className="form_data">
        <div className="form_heading">
          ...
          <p>Instance-based learning algorithm with 98.8% accuracy for anemia classification</p>
        </div>
