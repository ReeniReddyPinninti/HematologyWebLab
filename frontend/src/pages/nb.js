...
import { pred_nb } from '../services/Apis';
...
const NB = () => {
  ...
  const makePrediction = async (input) => {
    try {
      ...
      const responseAllFeatures = await pred_nb({gender, haemoglobin,mch, mchc, mcv,features: 'all',});
  ...
  const makeoptPrediction = async (input) => {
    try {
      ...
      const responseOptimized = await pred_nb({gender,haemoglobin,mcv,features:'optimized'});
    ...
    <section>
      <div className="form_data">
        <div className="form_heading">
          ...
          <p>Probabilistic classifier with 91.4% accuracy using Bayes' theorem</p>
        </div>
